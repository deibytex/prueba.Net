...
--EXEC EBUS.RellenoEbus_New @PeriodoFecha = '20220305',   @clienteIdS = 914,    @FechaInicial = null,     @FechaFinal = null 

--SELECT * FROM EBUS.Eficiencia_915 AS E

CREATE PROCEDURE [EBUS].[RellenoEbus_New]
    (
...
BEGIN
    ...
    BEGIN
        SET @mesAtras = -1;
        SET @FechaInicial = EOMONTH(@PeriodoFecha, @mesAtras);
    -- trae el ultimo dia del periodo anterior 
    ...
    IF (@FechaFinal IS NULL)
        SET @FechaFinal = DATEADD(DAY,  1, EOMONTH(@PeriodoFecha, 0));
    -- trae el ultimio dia del periodo actual
    ...
    (
        ...
        tripStart DATETIME,
        tripEnd DATETIME,
        distanceKilometers DECIMAL(18, 4),
        StartOdometerKilometers DECIMAL(18, 4),
        endOdometerKilometers DECIMAL(18, 4),
        Duracion INT
    );

    CREATE INDEX idxtempTrips
    ON #Trips (TripId)
    INCLUDE (
                assetId,
                driverId,
    ...
    BEGIN
      ...
      , fechasistema
         ...
         SELECT 
             ...
             , ClienteIds
             , fechasistema
            FROM [PORTAL].[TB_Event_' + @PeriodoW + N'_' + CAST(@clienteIdS AS VARCHAR(10)) + N'] (NOLOCK)  
            WHERE ( (@FechaI is null and @FechaF  is null) or  (StartDateTime >= @FechaI and  StartDateTime < @FechaF) )		  
		    --AND  (EsProcesado = 0 ) and isEbus = 1;
            
             -- Rellenamos Los trips
            INSERT INTO #Trips
            (
                TripId,
                assetId,
            ...
            SELECT TripId,
                ...
                endOdometerKilometers,
                Duracion
            FROM [PORTAL].[TB_Trips_' + @PeriodoW + N'_' + CAST(@clienteIdS AS VARCHAR(10)) + N'] (NOLOCK)
            WHERE ((@FechaI is null and @FechaF  is null) or  (tripStart >= @FechaI and  tripEnd < @FechaF) )		  
		    AND  (EsProcesado = 0 );
             ';
    ...
    -- EXEC EBUS.RellenoEficiencia_New @Clienteids = @clienteIdS,
    --                             @FormatTime = @FormatTime,
    --                             @PeriodoFecha = @PeriodoFecha,
    --                             @Periodoant = @Periodoant;

    -- -- Ejecutamos Relleno de ZP
    -- EXEC EBUS.RellenoZP_New @Clienteids = @clienteIdS,
    --                     @FormatTime = @FormatTime,
    --                     @PeriodoFecha = @PeriodoFecha,
    --                     @Periodoant = @Periodoant;

    -- -- Ejecutamos Relleno de Alarmas
    -- EXEC EBUS.RellenoAlarmas_New @Clienteids = @clienteIdS,
    --                          @FormatTime = @FormatTime,
    --                          @PeriodoFecha = @PeriodoFecha,
    --                          @Periodoant = @Periodoant;

    -- -- Ejecutamos Relleno de VIAJES 1 MIN
    -- EXEC EBUS.RellenoViajes1Min_New @Clienteids = @clienteIdS,
    --                             @FormatTime = @FormatTime,
    --                             @PeriodoFecha = @PeriodoFecha,
    --                             @Periodoant = @Periodoant;

    -- Ejecutamos Relleno de Recargas 30 Seg
    EXEC EBUS.RellenoRecargas30Seg_New @Clienteids = @clienteIdS,
                                   @FormatTime = @FormatTime,
                                   @PeriodoFecha = @PeriodoFecha,
                                   @Periodoant = @Periodoant;
