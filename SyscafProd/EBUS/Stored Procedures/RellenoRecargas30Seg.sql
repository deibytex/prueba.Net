...
-- Modified: 09.02.2022
-- =============================================
CREATE PROCEDURE [EBUS].[RellenoRecargas30Seg]
(
    @Clienteids INT,
...
BEGIN
    ...
    IF OBJECT_ID('tempdb..#FilterRecargas30Seg') IS NOT NULL
        DROP TABLE #FilterRecargas30Seg;
    CREATE TABLE #FilterRecargas30Seg
    (
        EventTypeId BIGINT NOT NULL
    );

    CREATE INDEX idxFilterRecargas30Seg
    ON #FilterRecargas30Seg (EventTypeId);

    -- Creamos tabla temp para evento de carga 30 seg
    
    -- Insertamos Datos del filtro
    INSERT INTO #FilterRecargas30Seg
    (
        EventTypeId
    )
    SELECT EventTypeId
    ...
    WHERE EventTypeId IN
          (
              ...
              WHERE TPDW.TipoPreferencia = 7
                    AND TPDW.Parametrizacion LIKE '%79%'
          );
    ...
    INSERT INTO EBUS.Recargas30seg_' + CAST(@Clienteids AS VARCHAR)
          + N' (
        Muestra,
        NoCarga,
        AssetId,
    ...
    INNER JOIN
        ...
        SELECT
          ...
          + N' AS E
            ...
            TE.EventId
        FROM #EventBus AS TE
        WHERE TE.EventTypeId = @EventCharguin 
            AND TE.TotalTimeSeconds > 30) AS NC            
    ...
    EXEC sp_executesql @SQLScript,
                       N'@FormatTime AS VARCHAR(10), @EventCharguin BIGINT, @EventSOC BIGINT, @EventCorriente BIGINT, @EventVoltaje BIGINT ',
                       @FormatTime,
                       @EventCharguin,
                       @EventSOC,
                       @EventCorriente,
