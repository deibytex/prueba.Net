...
CREATE PROCEDURE dbo.SP_RellenoErroresViajesyUso
(
...
BEGIN
    ...
    DECLARE @SQLScript NVARCHAR(MAX)
        = N' 
			IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''PORTAL.TB_Trips_' + @Periodo + '_' + CAST(@clienteIdS AS VARCHAR)
              + N''') AND type in (N''U''))
        INSERT INTO #Viajes
    ...
    SET @SQLScript
        = N'
		IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''PORTAL.TB_Event_' + @Periodo + '_' + CAST(@clienteIdS AS VARCHAR)
              + N''') AND type in (N''U''))
    ...
    DECLARE @FechaSistema DATETIME = DATEADD(HOUR, -5, GETDATE());
    ...
    SET @SQLScript
        = N' 
          ...
          + N']  set EsProcesado = 1
			...
			SELECT TEVU.TripNo
              FROM dbo.TB_ErroresViajesyUso AS TEVU
              WHERE TEVU.ClienteIdS = '+ CAST(@clienteIdS AS VARCHAR)+' AND FechaSistema = @FechaSistema
			)	 
             ';


    EXEC sp_executesql @SQLScript, N'@FechaSistema datetime', @FechaSistema;
