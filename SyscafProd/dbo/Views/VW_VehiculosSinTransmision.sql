...
CREATE VIEW dbo.VW_VehiculosSinTransmision
AS
SELECT TP.assetIdS,
       DATEDIFF(DAY, MAX(TP.Timestamp), DATEADD(HOUR, -5, GETDATE())) DiasSinTx,
...
GROUP BY TP.assetIdS,
         TS.diasTransmision
HAVING ((DATEDIFF(MINUTE, MAX(TP.Timestamp), DATEADD(HOUR, -5, GETDATE())) / 1440) >= ISNULL(TS.diasTransmision, 1));
