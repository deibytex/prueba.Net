...
-- Description: Genera la ifnormacion de las condiciones de señales po dia por cliente
-- =============================================

--exec SIG.GeneraSenialesPorDiaPorCliente '20220101', '20220101 23:59:59', 898

CREATE PROCEDURE SIG.GeneraSenialesPorDiaPorCliente
(
    @Fi DATETIME,
    @Ff DATETIME,
    @ClienteIds INT
...
BEGIN
    ...
    WHERE TC.clienteIdS = @ClienteIds;
    ...
    BEGIN
        ...
        SET @tablaname = @Periodo + '_' + CAST(@ClienteidsC AS VARCHAR);
        SET @cont = 0;
        ...
        SET @Sql
            = N'
					IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N''[Portal].[TB_Event_'
              + @tablaname
              + N']'') AND type IN (N''U''))
					...
					FROM PORTAL.TB_Event_' + @tablaname
              + N' (NOLOCK)
