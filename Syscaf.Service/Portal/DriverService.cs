...
namespace Syscaf.Service.Portal
{
    ...
    public class DriverService : IDriverService
    {
        ...
        {
            //// debe validr que la tabla a la que va a isnertar el mensaje exista            
            return await _conn.GetAllAsync(DriverQueryHelper._getByClientId, new { ClienteId, EsActivo }, commandType: CommandType.Text);
            // insertamos en la replica
        }
        public async Task<List<dynamic>> GetByClienteIds(int? ClienteIds, int? EsActivo)
        {
            //// debe validr que la tabla a la que va a isnertar el mensaje exista            
            return await _conn.GetAllAsync(DriverQueryHelper._getByClientIds, new { ClienteIds, EsActivo }, commandType: CommandType.Text);
            // insertamos en la replica
        }
    }
